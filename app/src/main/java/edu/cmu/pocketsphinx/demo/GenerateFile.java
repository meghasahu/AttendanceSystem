...
public class GenerateFile {
    ...
    public File saveDataDefaulters() {
        ...
        try {
            ...
            writer.append("NAME     \t ROLLNO \t  COURSE  \t ATTENDANCE \t");
            ...
            while (j < dmon.size()) {
                ...
                for (i = 1; i <= 31; i++) {
                    if (dmon.get(j).c[i].equals("present")) {
                        writer.append("P\t");
                    } else if (dmon.get(j).c[i].equals(null)||dmon.get(j).c[i].equals(" ")) {
                        writer.append("H\t");
    ...
    public File name() {
        ...
        File file = new File(Environment.getExternalStorageDirectory().getAbsolutePath(), tempname + ".txt");
        while (file.exists()) {
            random++;
